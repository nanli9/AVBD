...
class Viewer:
    def __init__(self, args):
        ...
        # Per-body HISTORY of recently-written gizmo positions. Used by
        # _on_drag to recognize a viser network echo (which carries the
        # exact value of a past write) vs. a real user drag. A single
        # "last written" snapshot isn't enough because echoes can arrive
        # 1–2 frames late: by the time the echo fires the on_update, the
        # body has already moved and we've written a NEWER position, so
        # the echo's value matches none of the *current* state and gets
        # misregistered as a drag → teleport. With a 60-entry deque (1 s
        # at 60 Hz) any plausible echo round-trip is still matchable.
        self._gizmo_write_history: dict[int, deque[np.ndarray]] = {}
    ...
    def _wire_drag(self, vb: ViewerBody, view_idx: int):
        ...
        def _on_drag(_evt):  # noqa: ANN001
            ...
            # Echo guard: viser's on_update fires asynchronously and may echo
            # back our programmatic writes 1–2 frames late. An echo carries
            # EXACTLY the value we wrote at some recent tick — so check
            # new_p against the per-body history of recent writes. Any match
            # within 1 mm (float-32 serialization noise) → echo, ignore.
            hist = self._gizmo_write_history.get(body_idx)
            if hist is not None:
                for ref in hist:
                    if np.linalg.norm(new_p - ref) < 1.0e-3:
                        return
            else:
                # No history yet (first interaction): compare against body.
                body_p = self.solver.positions()[body_idx]
                if np.linalg.norm(new_p - body_p) < 1.0e-3:
                    return
            ...
            with self.server.atomic():
                for vb in self.bodies:
                    if vb.tc is not None:
                        ...
                        vb.tc.position = p_t
                        hist = self._gizmo_write_history.setdefault(
                            vb.body.index, deque(maxlen=60),
                        )
                        hist.append(np.asarray(p_t, dtype=np.float32))
        finally:
            self._gizmo_suppress = False
    ...
    def _reset(self):
        ...
        self._gizmo_write_history.clear()
            ...
            with self.server.atomic():
                for vb in self.bodies:
                    ...
                    if drag_mode and vb.tc is not None:
                        # Don't fight the user: if a drag is in progress on
                        # THIS body, leave its gizmo where the user is
                        # holding it. Otherwise follow the body.
                        if vb.body.index in self._drag_targets:
                            continue
                        try:
                            ...
                            hist.append(np.asarray(p_render, dtype=np.float32))
                        except RuntimeError:
                            vb.tc = None
                try:
